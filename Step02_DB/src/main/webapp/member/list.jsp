...
<body>
	<%--
		...
		-포함된 jsp페이지 피라미터를 전달할 수 있다.
		-파라미터를 적절히 전달해서 포함된 jsp 페이지 내에서 해당 파라미터를 추출해서 어떤 로직을 수행할 수 있다.
		-아래같은 경우 thisPage라는 파라미터 명으로 member라는 문자열을 전달하는 것이다. 
   ...
   <div class="container">
      <a href="${pageContext.request.contextPath }/member/insertform.jsp" class="icon-link">
		...
		<span class="visually-hidden"> 회원추가</span> <!-- 눈에 보이지 않지만 탭이 왔을 때 읽어준다. 시각장애우을 위해 -->
	  </a>
      <h1>회원 목록 입니다.</h1>
      <table class="table table-striped">
         ...
         <%for(MemberDto tmp:list){ %>
            <tr>
               ...
               <td><%=tmp.getName() %></td>
               <td><%=tmp.getAddr() %></td>
               <td><a href="updateform.jsp?num=<%=tmp.getNum() %>" class="icon=link">
               ...
               <td><a href="delete2.jsp?num=<%=tmp.getNum() %>">삭제2</a></td>
               <td><a href="delete3.jsp?num=<%=tmp.getNum() %>">삭제3</a></td>
            </tr>
         <%} %>
         </tbody>
      </table>
