...
<body>
   ...
   <script>
   		...
   		//아래의 코드를 jQuery를 활용한 구조롤 변경해 보세요
   		
   		//id가 email 인 요소에 input 이벤트가 일어 났을떄 실행할 함수 등록
   		$("#email").on("input", function{
   				//$(this).removeClass("is-valid").removeClass("is-invalid"); 아래와 기능이 같다
...
--%>
      ...
      function checkPwd(){
         ...
         if(pwd != pwd2){
   		...
   		//pwd, #pwd2 모두를 선택해서 이벤트 리스너 함수등록
      ...
      $("#id").on("input", function(){
         ...
         const self=this;// 여기안에서 this는 document.querySelector("#id")
         ...
         .then(function(data){
            ...
            console.log(data);
            if(data.isExist){// 여기안에서 this는 fetch("checkid.jsp?inputId="+inputId)
      ...
      $("#signupForm").on("submit",function(event){
    	  ...
    	  //폼 전체의 유효성 여부
    	  const isFormaValid= isIdValid && isPwdValid && isEmailValid;
    	  //만일 폼이 유효하지 않으면
    	  if(!isFormValid){
    		  return false; //폼 전송을 막는다.(jquery 애서 submit 이벤트 리스너 함수 안에서 폼전송 막는 방법)
    	  }
      });
   </script>
</body>
