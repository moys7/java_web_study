...
<%
	/*
		전송 파라미터를 추출하면 숫자 모양을 띄고 있어도 문자열()으로 읽어와 진다.
		읽어와서 숫자(정수)로 바꾸고 싶으면 Integer 클래스의 static메소드 .parseInt() 메소드를 활용하면 된다.
...
<body>
	<p>
		<!-- 클라이언트에세 출력은 되지만 클라이언트(웹브라우저)가 무시하는 주석 -->
		<strong><%=num %></strong> 번 상품 <strong><%=amount %></strong>개를 주문했습니다.
		<%--
			jsp 페이지에서의 주석 : jsp 페이지가 무시하는 주석
			
			context path 는 나중에 제거되거나 변경될 예정이기 떄문에 java code 영역이나 jsp 페이지에
			실제로 코딩이 되어있으면 안된다.
